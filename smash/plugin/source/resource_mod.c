...
//One entry per file in a channel that has no resource id like bgm and cro
typedef struct
{
...
//DirectoryEntry is packed by FS. Its path is its first field, so copy it from
//the entry as bytes before passing an aligned local array to u16 readers.
...
//Writes the sdmc: prefix for whatever root supplies this resource into out,
//without a terminator, and returns how many bytes it wrote; 0 means no override
...
//The same answer for the two channels that have no resource id. Writes the
//finished path into out, which must hold out_size bytes, and answers its
//length, or 0 when no mod supplies the file or would not fit.
        ...
        if((OpenDirectory(&dir_handle, dirs[i]) & 0x80000000) == 0)
        {
            ...
            do
            {
                ...
                for(int j = 0; j < num_files_folders; j++)
                {
                    ...
                    {
                        //Paths are recorded relative to their own root, so the
                        //same game file supplied by two mods produces the same
                        //string and can be recognised as a conflict.
